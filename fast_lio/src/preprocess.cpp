...
Preprocess::Preprocess() : feature_enabled(0), lidar_type(AVIA), blind(0.01), det_range(100.0), max_height(5.0), point_filter_num(1)
{
  inf_bound = 10;
  N_SCANS = 6;
  SCAN_RATE = 10;
...
void Preprocess::velodyne_handler(const sensor_msgs::msg::PointCloud2::UniquePtr &msg)
{
  ...
  const double first_point_timestamp =
      pl_orig.points.front().timestamp;

  const double last_point_timestamp =
      pl_orig.points.back().timestamp;

  const bool timestamp_valid =
      std::isfinite(first_point_timestamp) &&
      std::isfinite(last_point_timestamp) &&
      first_point_timestamp > 0.0 &&
      last_point_timestamp >= first_point_timestamp;
  ...
  if (feature_enabled)
  {
    ...
    for (int i = 0; i < plsize; i++)
    {
      ...
      if (timestamp_valid)
      {
        const double relative_time_ms =
            (pl_orig.points[i].timestamp - first_point_timestamp) * 1000.0;

        added_pt.curvature =
            static_cast<float>(
                std::max(0.0, relative_time_ms));
      }
      else
      {
...
void Preprocess::default_handler(const sensor_msgs::msg::PointCloud2::UniquePtr &msg)
{
  ...
  /*** 从点坐标反算 azimuth 和 elevation，生成 curvature(时间偏移) 和 ring ***/
  // 先收集所有点的方位角以确定扫描起止角度
  std::vector<double> azimuths(plsize);
  for (uint i = 0; i < plsize; ++i)
  {
    azimuths[i] = atan2(pl_orig.points[i].y, pl_orig.points[i].x);  // [-π, π]
  }
  // 找到从负到正的角度跳变点（即扫描起始位置）
  double scan_period_ms = 1000.0 / SCAN_RATE;  // 扫描周期 (ms), 10Hz → 100ms
  double start_azimuth = -M_PI;  // 默认起始角度

  // 从点的顺序推断扫描方向：找到方位角跳变最大的位置
  double max_jump = 0;
  for (uint i = 1; i < plsize; ++i)
  {
    double jump = azimuths[i] - azimuths[i-1];
    if (jump < -M_PI) jump += 2*M_PI;  // 跨越 ±π 边界
    if (jump > M_PI)  jump -= 2*M_PI;
    if (fabs(jump) > fabs(max_jump))
    {
      max_jump = jump;
      start_azimuth = azimuths[i-1];  // 跳变前的位置是上一帧的结束
    }
  }

  // 前向扫描：计算每个点的时间偏移
  // curvature = 距扫描起始的时间 (ms)
  for(uint i = 0; i < plsize; ++i)
  {
    PointType added_pt;
    added_pt.normal_x = 0;
    ...
    // 将方位角映射到 [start_azimuth, start_azimuth + 2π)
    while (az < start_azimuth) az += 2*M_PI;
    while (az >= start_azimuth + 2*M_PI) az -= 2*M_PI;
    added_pt.curvature = (az - start_azimuth) / (2*M_PI) * scan_period_ms;
...
void Preprocess::robosenseM1_handler(const sensor_msgs::msg::PointCloud2::UniquePtr &msg)
{
  ...
  if (feature_enabled)
  {
    ...
    for (int i = 0; i < plsize; i++)
    {
      ...
      added_pt.normal_y = 0;
      added_pt.normal_z = 0;
      added_pt.curvature = (pl_orig.points[i].timestamp - start_time) * time_unit_scale;
...
void Preprocess::robosenseAiry_handler(const sensor_msgs::msg::PointCloud2::UniquePtr &msg)
{
  // If message type is XYZIRT (6+ fields), use robosenseM1_handler
  if (msg->fields.size() >= 6)
  {
    robosenseM1_handler(msg);
    return;
  }

  // XYZI mode: use voxel downsampling (solid-state flash LiDAR assumption)
  pl_surf.clear();
  pl_corn.clear();
  pl_full.clear();
  ...
  // Treat the whole scan as instantaneous (no deskewing)
  given_offset_time = true;
  ...
  for (const auto& ori_point : pl_downsampled->points)
  {
    ...
    if (range < det_range && range > blind && height_valid)
    {
      ...
      added_pt.normal_y = 0;
      added_pt.normal_z = 0;
      added_pt.curvature = 0.0;  // No deskewing for solid-state
